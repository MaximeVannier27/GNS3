...
def initAddressFamily(routerName,asName):
    ...
    if routerName.border:
        ...
        for i,c in routerName.interfaces: #récupérer toutes les interfaces des routeurs d'AS voisines, connectées a routeurName 
            if c[2] != routerName.AS_n:
                routeur_voisin = c[1]
                for j in routeur_voisin.interfaces.values():
                    if j[1] == routerName:
                        tmp = j[0]
                        break
